declare global {
  interface Window {
    ...
    scopeLoading: Record<
      string,
      { resolve: (module: any) => void; reject: (...args: any[]) => void }
    >;
  }
...
export interface Remote {
   ...
   * // using then
   *
   * remote().import("/apps/my-vue/assets/remoteEntry.js", "myVue", "./myVue", "module").then((module) => { module.exportedFunction();});
   *
   * // using await
   *
   * const module = await remote().import("/apps/my-vue/assets/remoteEntry.js", "myVue", "./myVue", "module");
   *
   ...
   */
  import: (url: string, scope: string, module: string, type?: "var" | "module") => Promise<any>;
   ...
   *
   * remote()
   *  .register("/apps/my-page/assets/remoteEntry.js", "myPage", "./myPage", "module");
   *  .register("/apps/my-vue/assets/remoteEntry.js", "myVue", ["./myVue", "./myComponent"], "module");
   *
   * @param url Url to the remote entry file
   * @param scope remote scope name
...
class RemoteImp implements Remote {
  ...
  register(url: string, scope: string, module: string | string[], type: "var" | "module" = "var") {
    if (!Array.isArray(module)) {
      module = [module];
    }

    for (const m of module) {
      this.getters[`${scope}/${m.replace(/^\.\//, "")}`] = () => this.import(url, scope, m, type);
    }
  ...
  async import(url: string, scope: string, module: string, type: "var" | "module" = "var") {
    ...
    if (!this.modules[`${scope}/${module}`]) {
      try {
        ...
        const error = (this.modules[`${scope}/${module}`] = new Error(
          `Cannot load module ${module} from ${scope} from ${url}`,
        ));
        console.error(err);
        console.error(error);
        throw error;
    ...
    if (this.modules[`${scope}/${module}`] instanceof Error)
      throw this.modules[`${scope}/${module}`];
  ...
  private fetchWebPackRemoteEntry(url: string, scope: string): Promise<ModuleFederationContainer> {
    const script = this.window.document.createElement("script");
    script.src = url;
    script.type = "text/javascript";
    script.async = true;

    const promise = new Promise<ModuleFederationContainer>((resolve, reject) => {
      script.onload = () => {
  ...
  private fetchModuleRemoteEntry(url: string, scope: string): Promise<ModuleFederationContainer> {
    const script = this.window.document.createElement("script");
    script.type = "module";
    script.innerHTML = `
      (async () => {
        try {
          ...
          if (window['scopeLoading']['${scope}']) {
            window['scopeLoading']['${scope}'].resolve(module);
            delete window['scopeLoading']['${scope}'];
          }
        } catch (error) {
          if (window['scopeLoading']['${scope}']) {
            window['scopeLoading']['${scope}'].reject(error);
            delete window['scopeLoading']['${scope}'];
          }
        }
      })();
    ...
    const promise = new Promise<ModuleFederationContainer>((resolve, reject) => {
      ...
      this.window["scopeLoading"][scope] = { resolve, reject };
    });
